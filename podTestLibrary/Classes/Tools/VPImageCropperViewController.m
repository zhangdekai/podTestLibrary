...
- (void)viewDidLoad {
    ...
    if (self.imageUrl) {
        ...
        [self.juHuaView setCenter:self.view.center];
        [tmpImageView sd_setImageWithURL:[NSURL URLWithString:self.imageUrl] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (image != nil) {
                self.originalImage = image;
                [self initView];
                [self.juHuaView stopAnimating];
                [self bringViewToForward];
            } else {
                [self showAlertWithMessage:@"图片URL无效"];
            }
        }];
    } else {
        [self initView];
...
- (void)initControlBtn {
    _cancelBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 60, 100, 60)];
    ...
    [_cancelBtn.titleLabel setLineBreakMode:NSLineBreakByWordWrapping];
    [_cancelBtn.titleLabel setNumberOfLines:0];
    [_cancelBtn setTitleEdgeInsets:UIEdgeInsetsMake(5.0f, 5.0f, 5.0f, 5.0f)];
    [_cancelBtn addTarget:self action:@selector(cancel:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:_cancelBtn];
    
    //
    self.juHuaView = [[UIActivityIndicatorView alloc] initWithFrame:CGRectMake(self.view.frame.size.width / 2 - 10, self.view.frame.size.height - 35.0f, 30, 30)];
    self.juHuaView.hidden = YES;
    self.juHuaView.hidesWhenStopped = YES;
    [self.view addSubview:self.juHuaView];
    
    
    confirmBtn = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width - 100.0f, self.view.frame.size.height - 60, 100, 60)];
    confirmBtn.backgroundColor = [UIColor clearColor];
    [confirmBtn setTitleColor:[UIColor mainColorBlue] forState:0];
    ...
    //底部灰条：
//    UIView *backView = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 60, ScreenWidth, 60)];
//    backView.backgroundColor = [UIColor ymh_colorWithHex:0x1d2225];
...
-(UIImage *)getSubImage{
    ...
    UIImage *last = [UIImage fixOrientation:smallImage];
